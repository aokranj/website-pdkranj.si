 ...
 <div class="flex items-center sidebar-width bg-slate-100 border-b border-slate-100">
     ...
     <template x-for="tab in constant.tabs">
         <a x-show="options.enable || ['channels'].includes(tab.id)" href="#" @click.prevent="setTab(tab.id)" class="group flex items-center gap-3 cursor-pointer px-3 py-3 hover:text-blue-500 transition duration-100 focus:ring-0 border-r border-slate-200 last:border-0" :class="{ 'bg-white text-blue-500 focus:text-blue-500': isTab(tab.id) }">
