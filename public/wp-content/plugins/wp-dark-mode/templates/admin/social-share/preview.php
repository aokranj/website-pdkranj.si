    ...
    <div class="w-full sm:sticky z-0 sm:top-40 flex flex-col gap-2 items-center justify-center">
        <div class="w-full flex items-center justify-center">
            ...
            <div x-show="!isTab('social-meta')" class="bg-slate-100 p-3 rounded-md shadow w-full border border-slate-200">
                ...
                <div class="font-semibold text-slate-600 text-sm mb-4">Preview</div>

                <div class="flex items-center justify-between gap-2 animate-pulse mb-2">
                ...
                <template x-for="index in options.button_position === 'both' ? 2 : 1">
                    <div class="w-full">
                        ...
                        <div x-show="options.button_position !== 'both' && index === 1 && ['both', 'below'].includes(options.button_position)" class="flex flex-col gap-2 w-full">
                            ...
                            <div class="flex items-center w-2/3 justify-between gap-2 animate-pulse">
                                <div class="h-7 w-full bg-slate-300 rounded-md"></div>
                                <div class="h-7 w-full bg-slate-300 rounded-md"></div>
                        ...
                        <section x-show="options.enable" class="my-6 _social-share-container" :class="{ 
                            ...
                            <!-- Share label  -->
                            <span x-show="options.share_via_label" class="_share-label focus:border-0 focus:outline-none focus:ring focus:ring-blue-500" x-text="options.share_via_label" contenteditable="true" @input="options.share_via_label = $event.target.innerText"></span>
                                ...
                                <template x-if="options.show_total_share_count">
                                    <div class="_total-share">
                                        <div class="_total-share-count" x-show="options.show_total_share_count">
                                            ...
                                            <span class="focus:outline-none focus:border-0 focus:ring focus:ring-blue-500 transition duration-75" x-text="options.shares_label || 'Share'" contenteditable="true" @input="options.shares_label = $event.target.innerText"></span>
                                        </div>
                                    </div>
                                </template>
            ...
            <!-- <div class="bg-slate-100 p-3 rounded-md shadow w-full border border-slate-200" x-show="isTab('social-meta')">
                <div class="text-sm text-slate-600 mb-2 font-semibold">Preview</div>
                <article class="bg-white p-3 rounded-sm border border-slate-200 transition duration-75" :class="{'opacity-50' : !options.enable_social_meta}">
                    <div class="w-full h-56 mb-1 rounded-sm flex items-center items-center">
                        <img class="w-full" :src="options.default_share_image || '<?php echo WP_DARK_MODE_ASSETS; ?>/images/default_share_image.png)'" alt="">
                    </div>
                    <p class="text-slate-400 uppercase text-xs"><?php echo parse_url(home_url(), PHP_URL_HOST); ?></p>
                    <h3 class="font-semibold text-base mb-1">Lorem ipsum olor sit.</h3>
                    <p class="text-xs text-slate-500">Lorem ipsum dolor, sit amet consectetur adipisicing elit. Odio quas suscipit at pariatur dolores unde dolorum at pariatur...</p>
                </article>
            </div> -->
        ...
        <div class="flex justify-end gap-2 mt-2">
            <button type="button" class="px-4 py-2 rounded-sm text-sm transition duration-75" @click.prevent="saveOptionsButton" :class="{ 'bg-green-500 hover:bg-green-600 text-white pl-2' :isChanged, 'bg-slate-200 cursor-not-allowed text-slate-500 pointer-events-none' : !isChanged }">
                <span class="flex items-center gap-1">
                    <svg x-show="state.isChanged" xmlns="http://www.w3.org/2000/svg" class="fill-current w-5" viewBox="0 0 16 16">
                        <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                    </svg>
                    <span x-text="state.isChanged ? 'Save and Publish' : options.enable ? 'Published' : 'Saved'"></span>
                </span>
            </button>
        </div>
    </div>
